...
class QuartOAuthTest(IsolatedAsyncioTestCase):
    ...
    async def test_init_app_params(self):
        oauth = OAuth()
        oauth.init_app(self.app, SimpleCache())
    ...
    async def test_register_oauth1_remote_app(self):
        ...
        oauth = OAuth(self.app, cache=SimpleCache())
        oauth.register("dev", **client_kwargs)
        self.assertEqual(oauth.dev.name, "dev")
        self.assertEqual(oauth.dev.client_id, "dev")
    ...
    async def test_oauth1_authorize_cache(self):
        self.app.secret_key = "!"
        cache = SimpleCache()
        oauth = OAuth(self.app, cache=cache)
        ...
        async with self.app.test_request_context("/?oauth_token=foo"):
            with mock.patch("requests.sessions.Session.send") as send:
                send.return_value = mock_send_value(
    ...
    async def test_oauth2_authorize(self):
        ...
        async with self.app.test_request_context(path=f"/?code=a&state={state}"):
    ...
    async def test_access_token_with_fetch_token(self):
        ...
        oauth = OAuth()
        token = get_bearer_token()
        oauth.init_app(self.app, fetch_token=lambda name: token)
        client = oauth.register(
            "dev",
            client_id="dev",
            client_secret="dev",
    ...
    async def test_request_with_refresh_token(self):
        ...
        oauth.init_app(self.app, fetch_token=lambda name: expired_token)
        client = oauth.register(
            "dev",
            client_id="dev",
            client_secret="dev",
